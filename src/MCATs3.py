...
class ActiveTurbulenceSimulation:
    ...
    def run(self, T, save_interval=1.0, verbose=True):
        """
    ...
    def reset(self):
        ...
        self.positions = np.random.uniform(0, self.L, (self.N, 2))
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("QUICK TEST: Magnetically Controlled Active Turbulence")
    print("="*70 + "\n")
    
    # Test 1: Baseline turbulence
    print("Test 1: Baseline turbulence (B=0)")
    sim = ActiveTurbulenceSimulation(N=200, L=200.0, dt=0.01)
    sim.set_magnetic_field(B=0.0)
    sim.run(T=10.0, save_interval=1.0)
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    sim.plot_snapshot(ax=ax1, title='Baseline Turbulence (B = 0 mT)')
    sim.plot_order_parameter(ax=ax2)
    plt.tight_layout()
    plt.savefig('test_baseline.png', dpi=150)
    print("Saved: test_baseline.png\n")
    
    # Test 2: Magnetic suppression
    print("Test 2: Magnetic suppression (B=25 mT)")
    sim.reset()  # Reset to new random state
    sim.set_magnetic_field(B=25.0)
    sim.run(T=10.0, save_interval=1.0)
    ...
    sim.plot_order_parameter(ax=ax2)
    plt.tight_layout()
    plt.savefig('test_suppressed.png', dpi=150)
    print("Saved: test_suppressed.png\n")
    
    print("="*70)
    print("TESTS COMPLETE!")
    print("="*70)
    print("\nGenerated files:")
    print("  - test_baseline.png")
    print("  - test_suppressed.png")
    print("\nSimulation class is working correctly!")
